  ...
  config = mkIf cfg.enable (mkMerge [
      ...
      home.packages = with pkgs; [
        ...
        (writeShellScriptBin "e" ''
          exec ${cfg.package}/bin/emacsclient \
            --socket-name=${cfg.server.socketName} \
            --no-wait \
            --alternate-editor="" \
            "$@"
        '')
    ...
    (mkIf cfg.defaultEditor {
      home.sessionVariables = {
        EDITOR = "emacsclient --socket-name=${cfg.server.socketName} --tty --alternate-editor=''";
        VISUAL = "emacsclient --socket-name=${cfg.server.socketName} --create-frame --alternate-editor=''";
      };
    })
    
    # Shell integrations
    (mkIf (cfg.server.enable && cfg.enableBashIntegration) {
      programs.bash.shellAliases = {
        emacs = "emacsclient --socket-name=${cfg.server.socketName} --create-frame --alternate-editor=";
      };
    })
    
    (mkIf (cfg.server.enable && cfg.enableZshIntegration) {
      programs.zsh.shellAliases = {
        emacs = "emacsclient --socket-name=${cfg.server.socketName} --create-frame --alternate-editor=";
      };
    })
    
    (mkIf (cfg.server.enable && cfg.enableFishIntegration) {
