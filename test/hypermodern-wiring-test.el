...
(ert-deftest hypermodern/test-avy-keybinding-line ()
  "Avy line jump keybinding (if configured)."
  (skip-unless (not noninteractive))
  ;; M-g l is optional, not configured in init.el
  (should t))

(ert-deftest hypermodern/test-avy-keybinding-word ()
  "Avy should have word jump keybinding."
  (skip-unless (not noninteractive))
  (should (eq (key-binding (kbd "C-;")) 'avy-goto-word-1)))
...
(ert-deftest hypermodern/test-deadgrep-keybinding ()
  ...
  (skip-unless (not noninteractive))
  (let ((binding (key-binding (kbd "C-c s"))))
...
(ert-deftest hypermodern/test-magit-loadable ()
  "Magit should be loadable (if git is available)."
  ;; Magit requires git to be available during loading
